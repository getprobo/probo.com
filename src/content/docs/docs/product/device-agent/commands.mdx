...
- **macOS:** install the signed `.pkg`, then enroll from the Probo console via `Probo Agent.app` / `probo://` deeplink. See [macOS](/docs/product/device-agent/macos).
- **Windows:** install the signed `.msi`, then enroll from the Probo console via the tray. See [Windows](/docs/product/device-agent/windows).
- **Linux:** enroll with `install.sh` or `probo-agent install`. See [Linux](/docs/product/device-agent/linux).
- **FreeBSD:** enroll with `install.sh` or `probo-agent install`. See [FreeBSD](/docs/product/device-agent/freebsd).
