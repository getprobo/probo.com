...
- **Token rejected.** Qovery answers a missing or invalid token with a 401. Confirm the value starts with `qov_`, or `sk-qov-` for an API policy token, and is an organization token that is still listed under **Settings** > **API token**, and not a Console session token, which the API accepts only as a `Bearer` credential.
