...
description: How Probo stores and uses API keys, OAuth tokens, and workload identity for access sources, and what controls protect your data on Probo Cloud
---
...
Access reviews support these connection methods:

- **OAuth.** When a provider offers **OAuth** on the **Connections** page, you approve Probo on the provider's consent screen. Probo stores OAuth tokens needed to refresh access, not your provider password. OAuth availability depends on the provider and deployment configuration.
- **API key.** You create a token on the provider and paste it into Probo when you connect it. Use the least-privilege token each connector guide describes.
- **Workload identity.** Used by [Amazon Web Services](/docs/product/access-review/aws). You create a read-only IAM role in your AWS account, and Probo assumes it with a short-lived token that it signs in memory. The only value Probo stores is the Role ARN, which is not a secret.
- **CSV.** No live credential is stored; you upload an export for tools Probo cannot connect to directly.

For OAuth and API key flows, Probo must hold credentials that can be used again on each sync or campaign. That is why those credentials are **encrypted at rest**, not hashed like a password. Workload identity holds no such credential, so it needs no encrypted credential blob.

## Data flow

The sequence diagram below follows the same lifecycle as the numbered steps that follow. Messages read top to bottom within each section. **CSV** uploads skip credential storage, and workload identity stores only a Role ARN. The connect, store, and sync path applies to OAuth and API keys.
...
Some integrations use a **deployment-managed token** (for example the Crisp marketplace plugin). In that model Probo does not copy the shared plugin token into your organization's stored connection. Instead you approve an app installation at the provider, and Probo confirms that installation with the provider before the connection completes. Check the connector guide for your provider.
...
- Use **workload identity** for Amazon Web Services. For other providers, prefer **OAuth** when the **Connections** page offers it.
...
Some providers can restrict API credentials to specific source IP addresses. For **Probo Cloud**, allowlist the [public egress address](/docs/deployment/ip-ranges) for your region, or leave the restriction disabled. For a **self-hosted deployment**, allowlist the fixed egress addresses configured by your infrastructure operator.
...
- [Amazon Web Services](/docs/product/access-review/aws)
- [Public egress IP addresses](/docs/deployment/ip-ranges)
